...
class JointMisinformationOptimiser(BaseRetriever):
    ...
    def __init__(
        ...
        llm_name: str | None = None,   # <-- allow disabling LLM
        device: torch.device = None,
        seed: int = 123,
    ) -> None:
        """
        Initialise the optimiser with retrieval and (optional) LLM components.

        If llm_name is None, the LLM is NOT loaded. In that case you must pass
        suffix_ids into generate_joint_trigger_and_passage (so no suffix generation
        is attempted).

        Args:
            retriever_name (str): Name of the retriever model to be attacked.
            llm_name (str | None): Name of the language model used to generate suffix content,
                                   or None to disable LLM loading.
        ...
        # Lazily/optionally load LLM (avoids gated repo access when suffix is precomputed)
        ...
        # Use a forward hook + role flag to disambiguate sources
